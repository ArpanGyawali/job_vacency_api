...
const userMyProfile = async(user, role, res) => {
  try {
    let profile
    if (role === 'seeker'){
      profile = await SeekerProfile.findOne({ user: user._id }).populate('users', ['name', 'username', 'avatar', 'role'])
    }
    else if (role === 'recruiter'){
      profile = await RecruiterProfile.findOne({ user: user._id }).populate('users', ['name', 'username', 'avatar', 'role'])
    }
    if(!profile) {
...
const userProfileUpdate = async(req, role, res) => {
  ...
  if (jobInterests){
    profileFields.jobInterests = jobInterests.split(',').map(interest => interest.trim())
  }
...
const getById = async(req, res, database) => {
  try {
    const profile = await database.findOne({ user: req.params.user_id }).populate('users', ['name', 'avatar']) 
    if(!profile){
      return res.status(400).json({
