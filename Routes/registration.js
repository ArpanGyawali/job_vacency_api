...
const validationArr = [
   ...
   check('username', 'Must contain only letters, numbers and an optional underscore in between and must start with a letter').matches(/^[a-z][a-z\d]*_?[a-z\d]+$/i),
   check('email', 'Email is required').not().isEmpty(),
   check('email', 'Enter a valid email address').isEmail(),
   check('password', 'Password must at least be 7 characters long').isLength({ min: 7 })
]
